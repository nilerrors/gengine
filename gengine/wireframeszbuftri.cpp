...
ZBuffering::ZBuffering(const ini::Configuration &config, bool lighted) : ZBufferedWireframe(config, lighted)
{
...
const img::EasyImage &ZBuffering::drawWireframe()
{
    ...
    for (Figure *figure: figures)
    {
        for (Face &face: figure->faces)
        {
            ...
            zBuffer.draw_zbuf_triag(image, A, B, C, scale_factor, d_x, d_y, figure->ambientReflection,
                                    figure->diffuseReflection, figure->specularReflection,
